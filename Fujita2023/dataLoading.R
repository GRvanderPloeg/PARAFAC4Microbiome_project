...
write.csv(df, "./Data/16S_data.csv", row.names=FALSE)
write.csv(sampleInfo, "./Data/sampleInfo.csv", row.names=FALSE)
write.csv(Taxa_list %>% as_tibble(), "./Data/taxonomy.csv", row.names=FALSE)

library(ggpubr)
totalSums = rowSums(df, na.rm=TRUE)
relAbs = sweep(df, 1, totalSums, FUN="/")

plotAllReplicates = function(treatmentName){
  plotlist = list()
  for(i in 1:8){
    plotlist[[i]] = relAbs %>% 
      as_tibble() %>% 
      mutate(sampleID = sampleInfo$sampleID) %>% 
      left_join(sampleInfo) %>%
      filter(treat1==treatmentName,replicate.id==i) %>%
      select(-treat1,-replicate.id,-treat2,-missing,-timeChar,-resource,-inoculum,-sampleID) %>%
      pivot_longer(-time) %>% 
      mutate(ID=name) %>% 
      select(-name) %>% 
      left_join(Taxa_list) %>% 
      ...
      ggplot(aes(x=as.factor(time),y=s,fill=as.factor(Genus))) + 
      geom_bar(position="stack",stat="identity",colour="black") + 
      theme(legend.position="none")
  }
  plot = ggarrange(plotlist=plotlist, nrow=8)
  print(plot)
}